...
Agent 自身维护迭代状态，编排 "转译/生成 → 统一调试 (ascendc-debugger)" 的循环。
...
while ac_iteration < max_ac_iterations:
    ...
    重试 (ac_iteration > 0):
      传入: output_dir + ac_debugger_error_summary
      根据 debugger 的失败报告重新转译或修改 kernel/ 和/或 model_new_ascendc.py

    产物 → {output_dir}/model_new_ascendc.py
           {output_dir}/kernel/

    ── 4.2 统一调试 (ascendc-debugger) ───────────────
    调用 ascendc-debugger skill 进行自动诊断与修复

    传入: task_name={task_dir_name} workdir={workdir} soc_version={ASCENDC_SOC_VERSION}

    debugger 内部执行:
      1. 运行 diagnose.py (退化检测 → 编译 → 验证)
      2. 如失败，读取全部上下文 (design/tile_level/, design/block_level/, model.py, kernel/*.cpp, model_new_ascendc.py)
      3. 分析根因，生成 debug audit 与 fix plan
      4. 修改 kernel/*.cpp 和/或 model_new_ascendc.py
      5. 重新编译验证，最多 5 轮内部迭代

    debugger 返回结果:
      PASS → break，Phase 4 成功，进入 Phase 5
      STOP →
        如果 ac_iteration < max_ac_iterations - 1:
          ac_debugger_error_summary = debugger 的失败报告摘要
...
当 ascendc-debugger 返回 STOP 时，提取以下信息用于下一轮重试：

```
失败模式: <degenerate|compile|precision|verify>
失败摘要: <一句话描述>
根因: <根因分析>
已尝试修复: <修复内容>
剩余问题: <仍未解决的关键问题>
...
| Phase 4 | AscendC 退化/编译/验证失败 | 调用 ascendc-debugger skill 自动诊断修复 |
...
| 分类 | 含义 | 处理 |
|------|------|------|
| A 类 — 代码逻辑/算法错误 | 可修复，含退化子类型 | 由 ascendc-debugger 自动处理 |
| A-TileLangFallback-Type{1-4} | TileLang 实现退化（见 Phase 3 子类型表） | 按退化脚本 suggestion 修复 |
| A-AscendCFallback-Type{1-4} | AscendC 实现退化（见 Phase 4 子类型表） | 由 ascendc-debugger 自动处理 |
