...
class BuildExt(build_ext):
    def run(self):
        so_files = glob.glob(str(BUILD_DIR / "matmul_leakyrelu_ext*.so"))
        if not so_files:
...
setuptools.setup(
    ...
    version="0.1.0",
    description="matmul_leakyrelu AscendC kernel — registers torch.ops.npu.matmul_leakyrelu",
    ext_modules=[NpuExtension("matmul_leakyrelu_ext", sources=[])],
