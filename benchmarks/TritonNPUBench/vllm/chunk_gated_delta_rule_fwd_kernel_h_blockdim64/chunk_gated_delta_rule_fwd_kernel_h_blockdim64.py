...
# Test file: /home/z00841464/benchmark/Triton_Automation-br_a3_operator/ascend/test/vllm_operator_cases/newtest_cases/test_chunk_gated_delta_rule_fwd_kernel_h_blockdim64.py
# Main kernel: chunk_gated_delta_rule_fwd_kernel_h_blockdim64
# PT file: chunk_gated_delta_rule_fwd_kernel_h_blockdim64_v2.pt
...
def chunk_gated_delta_rule_fwd_kernel_h_blockdim64(
    ...
    if USE_INITIAL_STATE:
        ...
        b_h1 += tl.load(p_h0_1, boundary_check=(0, 1), padding_option='zero').to(tl.float32)
        if K > 64:
            p_h0_2 = tl.make_block_ptr(
                h0, (K, V), (V, 1), (64, i_v * BV), (64, BV), (1, 0)
            )
            b_h2 += tl.load(p_h0_2, boundary_check=(0, 1), padding_option='zero').to(tl.float32)
        if K > 128:
            p_h0_3 = tl.make_block_ptr(
                h0, (K, V), (V, 1), (128, i_v * BV), (64, BV), (1, 0)
            )
            b_h3 += tl.load(p_h0_3, boundary_check=(0, 1), padding_option='zero').to(tl.float32)
        if K > 192:
            p_h0_4 = tl.make_block_ptr(
                h0, (K, V), (V, 1), (192, i_v * BV), (64, BV), (1, 0)
            )
            b_h4 += tl.load(p_h0_4, boundary_check=(0, 1), padding_option='zero').to(tl.float32)
