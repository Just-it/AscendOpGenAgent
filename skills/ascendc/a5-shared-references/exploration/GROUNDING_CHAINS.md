...
- **Note**: If vec_ratio > 90% AND GPU/NPU matches core count ratio (108/56=1.93), the gap is architectural — do not explore
...
2. GPU/NPU ratio matches core count ratio within 20% (e.g., 108 SMs / 56 AIV cores = 1.93x)
3. Theoretical compute throughput ratio matches observed performance ratio

If all true → performance gap is architectural, not algorithmic. No code transformation will close it.
