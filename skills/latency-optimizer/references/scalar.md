...
indices = tl.load(indices_ptr + offsets, mask=mask)  # vector 加载索引
# 对于 gather 操作，使用 vector 索引仍优于标量循环
for j in range(SUB_BLOCK):
    val = tl.load(input_ptr + indices[j])
    tl.store(output_ptr + start + j, val)
```
...
```python
is_invalid_tok = tok.to(tl.float16) < 0               # 转换float16类型
valid_block = (block_id.to(tl.float16) < max_num_blocks_per_req) & (block_id.to(tl.float16) >= 0)   # vector比较
```
...
4. **分支消除**：使用 `tl.where` 替代 if-else，避免标量控制流导致的 SIMD divergence
5. **内存对齐**：确保 BLOCK_SIZE 满足 128 字节对齐（如 FP16 类型下 BLOCK_SIZE 为 64 的倍数），以启用 vector 加载/存储
6. **类型转换**：对于 tl.where 中的整数索引比较，先 .to(tl.float32) 再比较，以启用向量比较指令。
