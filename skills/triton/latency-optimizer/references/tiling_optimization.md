...
处理多维张量（3D 及以上）的规约类（Reduction）、归一化类（Normalization）或数据搬运类算子，且还原轴（Reduction Axis）或搬运方向并非内存布局中的最连续轴（通常为最后一维 N）。
...
5. **数据搬运类算子特化**：对于 split/chunk/slice 等数据搬运操作，检查是否按内存最连续轴进行向量化。当操作维度后无其他维度（`inner_size == 1`）时，可直接向量化读取该维度；否则向量化 inner 维度。
